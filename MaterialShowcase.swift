...
public protocol ShowcaseDelegate {
    func dismissed()
}
...
public class MaterialShowcase: UIView {
    ...
    // MARK: Delagate
    public var delegate:ShowcaseDelegate?
...
extension MaterialShowcase {
    ...
    // Finally, shows it
    public func show(id : String , completion handler: (()-> Void)?) {
        
        var isFind : Bool = false
        
        if(SaveAndLoadModel().load(entity: "SHOWCASE") != nil){
        
        for r in SaveAndLoadModel().load(entity: "SHOWCASE")!{
            print(r.value(forKey: "id"))
            if(r.value(forKey: "id") as! String == id){
